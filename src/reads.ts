...
export function isDuplicate(record, target, stat) {
  ...
  if (record.path !== target.path) return false;
  if (record.offset !== target.offset || record.limit !== target.limit) return false;
 ...
 */
export function createReadTracker(options: { maxSessions?: number; maxWindows?: number } = {}) {
