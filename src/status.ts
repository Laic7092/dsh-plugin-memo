 ...
 */
import { readText, isFile, stamp, writeTextAtomic } from "./store.ts";
...
export function statusTemplate(projectName, at = stamp()) {
  ...
  for (const title of STATUS_SECTIONS) lines.push("", `## ${title}`, "");
  return `${lines.join("\n")}\n`;
}
...
function withUpdated(preamble, at) {
  if (UPDATED.test(preamble)) return preamble.replace(UPDATED, `_最后更新：${at}_`);
...
export function readStatus(paths) {
  if (!isFile(paths.status)) return { present: false, text: "", sections: [], updated: null, error: null };
  const file = readText(paths.status);
  if (!file.ok) return { present: false, text: "", sections: [], updated: null, error: file.error };
