...
(window as unknown as {
	...
	factory: (require) => {
		...
		function readStoredConfig() {
			...
			try {
				...
				if (parsed.subcommands !== null && typeof parsed.subcommands === "object" && !Array.isArray(parsed.subcommands)) {
					const subcommands = {};
					for (const [name, on] of Object.entries(parsed.subcommands)) if (typeof on === "boolean") subcommands[name] = on;
					if (Object.keys(subcommands).length > 0) kept.subcommands = subcommands;
				}
				return kept;
			} catch {
				return {};
		...
		function commandMap(config) {
			const map = {};
			if (config === null || typeof config !== "object" || !Array.isArray(config.subcommands)) return map;
			for (const group of config.subcommands) {
				if (group === null || typeof group !== "object" || !Array.isArray(group.commands)) continue;
				for (const entry of group.commands) if (entry && typeof entry.name === "string") map[entry.name] = entry.on === true;
			}
			return map;
		}
		...
		function patchFor(wanted, live) {
			...
			if (Array.isArray(live.subcommands) && wanted.subcommands !== null && typeof wanted.subcommands === "object") {
				...
				const subcommands = {};
				for (const [name, on] of Object.entries(wanted.subcommands)) if (name in known && typeof on === "boolean") subcommands[name] = on;
				if (Object.keys(subcommands).length > 0) patch.subcommands = subcommands;
			}
			return patch;
		}
		...
		function sameConfig(patch, live) {
			...
			if ("subcommands" in patch) {
				...
				for (const [name, on] of Object.entries(patch.subcommands)) if (liveCommands[name] !== on) return false;
			}
			return true;
		}
		 ...
		 */
		function ask(path: string, options: { root?: string; refresh?: boolean; tokenizer?: string; method?: string; body?: any } = {}) {
		...
		function indexCard(index) {
			if (index.present !== true) return card("代码索引", prose("还没有索引。在项目里跑一次 memo scan 就会建起来。", "memoMuted"));
		...
		function statusCard(status) {
			if (status.present !== true) return card("STATUS.md", prose("还没有 STATUS.md。让模型跑一次 memo handoff 就有第一份。", "memoMuted"));
		...
		function commandsCard(config, saving, save) {
			...
			blocks.push(
				h(
					...
					"关掉的子命令会被 CLI 拒绝：模型调 memo 时拿到一句拒绝的理由，/memo 也一样。重启 profile 后回到 composition 里的值，这个页面会在下次打开时重新应用你选过的。",
				),
			);
