...
| 工具 | 读写 | 用途 |
|---|---|---|
| `memo_scan` | **写** | 重建代码索引：文件、行数、估算 tokens、开头注释当描述、符号+行范围、按 import 图排名 |
...
`readGuard` / `refresh` / `exclude` / `tools` 在 Memo 视图里也能改，但**只改宿主进程的内存**：重启回到
