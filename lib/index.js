...
import { MEMO_COMMAND_NAMES, MEMO_COMMANDS, runMemo } from "./cli.js";
import { panelConfig, panelScan, panelState } from "./panel.js";
import { createReadTracker, DEFAULT_READ_TOOLS, readTarget } from "./reads.js";
import { DEFAULT_DIR, statOrNull, stamp } from "./store.js";
import { TOKEN_MODES } from "./indexer.js";
...
export function apply(ctx, config = {}) {
    ...
    for (const commandName of Object.keys(pinned)) {
        if (!MEMO_COMMAND_NAMES.includes(commandName))
            log(`config.subcommands has an unknown command ${JSON.stringify(commandName)} — ignored`);
    }
    ...
    const analyzerFor = () => {
        if (analyzerPromise === null)
            analyzerPromise = createTsAnalyzer();
        return analyzerPromise;
    };
    analyzerFor()
        .then((analyzer) => {
        if (analyzer === null)
            log("code index: line-based extraction only (tree-sitter is unavailable)");
        else
            log(`code index: tree-sitter upgrade available for ${analyzer.grammars.join(", ")}`);
    })
        .catch(() => { });
    ...
    const setCommand = (commandName, enabled) => {
        if (!Object.hasOwn(state.subcommands, commandName))
            return false;
    ...
    ctx.inject(["webServer"], (webCtx) => {
        ...
        webCtx.effect(() => webCtx.webServer.register({
            ...
            async handler(req, res) {
                try {
                    ...
                    const value = await panelState(params.get("root"), 
        ...
        webCtx.effect(() => webCtx.webServer.register({
            ...
            async handler(req, res) {
                if (req.method !== "POST")
                    return send(res, 405, { ok: false, error: "POST /memo/scan" });
                try {
                    const value = await panelScan(paramsOf(req).get("root"), state, await analyzerFor());
                    send(res, value.ok ? 200 : 400, value);
                }
                catch (error) {
                    fail(res, error);
        ...
        webCtx.effect(() => webCtx.webServer.register({
            ...
            async handler(req, res) {
                try {
                    if (req.method === "GET")
                        return send(res, 200, { ok: true, config: panelConfig(state) });
                    if (req.method !== "POST")
                        return send(res, 405, { ok: false, error: "GET or POST /memo/config" });
                    const body = await readBody(req);
                    if (!body.ok)
                        return send(res, 400, { ok: false, error: body.error });
                    ...
                    if (typeof wanted.readGuard === "boolean")
                        setGuard(wanted.readGuard);
                    if (typeof wanted.refresh === "boolean")
                        state.refresh = wanted.refresh;
                    if (wanted.tokenizer !== undefined) {
                        ...
                        if (changedCounter)
                            log(`token counter is now ${state.tokenizer}; the next scan or refresh rebuilds <project>/.memo/index.json`);
                    }
                    ...
                    if (wanted.subcommands !== undefined) {
                        ...
                        for (const [commandName, enabled] of entries) {
                            if (typeof enabled !== "boolean")
                                return send(res, 400, { ok: false, error: `subcommands.${commandName} must be a boolean` });
                            if (!Object.hasOwn(state.subcommands, commandName))
                                return send(res, 400, { ok: false, error: `unknown command ${JSON.stringify(commandName)}` });
                        }
                        for (const [commandName, enabled] of entries)
        ...
        guardEffect = ctx.effect(() => {
            ...
            disposers.push(ctx.on("session/event", (session, event) => {
                try {
                    ...
                    if (type.startsWith("compaction/") && session && typeof session.id === "string")
    ...
    const off = MEMO_COMMAND_NAMES.filter((commandName) => state.subcommands[commandName] === false);
    log(`memo CLI: 1 tool, ${MEMO_COMMAND_NAMES.length - off.length}/${MEMO_COMMAND_NAMES.length} subcommands on${off.length > 0 ? ` (off: ${off.join(", ")})` : ""}; project memory in <project>/${state.dirName}/`);
