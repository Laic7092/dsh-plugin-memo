...
async function openIndex(paths, state, analyzer) {
  ...
  let fresh;
  try {
    fresh = await refreshIndex(loaded.index, { exclude: state.exclude, analyzer });
  } catch (error) {
...
function renderScan(paths, index, stale, durationMs) {
  ...
  const out = [
    `已重建索引 · ${paths.index}`,
    `${fmt(meta.fileCount)} 个文件 · ${fmt(meta.symbolCount)} 个符号 · 约 ${fmt(meta.totalTokens)} tokens · 用时 ${durationMs} ms`,
...
function renderMap(map) {
  if (map.mode === "rollup") {
    ...
    const out = [`项目地图（按目录）· 约 ${fmt(map.spent)}/${fmt(map.budget)} tokens`, ""];
    for (const bucket of map.dirs) {
      out.push(`${bucket.dir}/  ${bucket.files} 个文件 · 约 ${fmt(bucket.tokens)} tokens · 代表：${bucket.best ? bucket.best.relPath : "-"}`);
    }
  ...
  const out = [
    `聚焦地图 "${map.focus}" · ${map.files.length}/${fmt(map.total)} 个文件 · 约 ${fmt(map.spent)}/${fmt(map.budget)} tokens${map.truncated ? "（被预算截断）" : ""}`,
    "",
  ];
  for (const file of map.files) {
    out.push(`${file.relPath}  [${Number(file.importance ?? 0).toFixed(2)}]  ${file.symbols} 符号 · 约 ${fmt(file.tokens)} tokens`);
...
export function apply(ctx, config = {}) {
  ...
  registerMemoTool(defineTool({
    name: "memo_scan",
    description:
      "Build this project's code index (<project>/.memo/index.json): every source file with its line count, estimated tokens, opening-comment description, symbols with exact line ranges, and an importance score from the import graph. WRITES the index. Run it once to create the index; afterwards memo_find and memo_map keep it current on their own, so reach for this again to rebuild from scratch (after a big refactor, a move, or if the index ever looks wrong).",
    parameters: {
      ...
      async handler(req, res) {
        try {
          ...
          const value = await panelState(
            ...
            params.get("refresh") === "1" ? { ...state, refresh: true } : state,
            await analyzerFor(),
          );
    ...
    webCtx.effect(() => webCtx.webServer.register({
      ...
      async handler(req, res) {
        try {
          ...
          log(`config changed: readGuard=${state.readGuard} refresh=${state.refresh} exclude=[${state.exclude.join(", ")}] toolsOff=[${off.join(", ")}]`);
