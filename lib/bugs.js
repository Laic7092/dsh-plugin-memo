 ...
 */
import { FORMAT_VERSION, collapse, isFile, readJson, stamp, writeJsonAtomic } from "./store.js";
...
function bugArray(value) {
    if (Array.isArray(value))
        return value;
    if (value && typeof value === "object" && Array.isArray(value.bugs))
        return value.bugs;
    return null;
}
export function loadBugs(paths) {
    ...
    if (!file.ok)
        return { ok: false, present: true, bugs: [], error: file.error };
...
export function appendBug(paths, entry, at = stamp()) {
    ...
    if (symptom.length === 0)
        return { ok: false, error: "error_message is required" };
    ...
    if (existing) {
        ...
        if (entry.rootCause)
            existing.root_cause = collapse(entry.rootCause);
        if (entry.fix)
            existing.fix = collapse(entry.fix);
        if (entry.file)
            existing.file = collapse(entry.file);
...
export function searchBugs(bugs, term, limit = 5) {
    ...
    for (const bug of bugs) {
        ...
        const haystack = normalizeSymptom([bug.error_message, bug.root_cause, bug.fix, (bug.tags ?? []).join(" "), bug.file].join(" "));
        let score = 0;
        if (symptom.includes(needle))
            score += 20;
    ...
    scored.sort((a, b) => b.score - a.score || String(b.bug.last_seen ?? "").localeCompare(String(a.bug.last_seen ?? "")));
