 ...
 *
 * The line-based extractor in `indexer.js` is the floor: it always runs, it
 ...
 *
 * Godot is the exception that proves the rule: `tree-sitter-wasms` carries
 * neither of its languages, so both are compiled and shipped inside the plugin
 * (see `grammarCandidates`), while the package stays optional elsewhere.
 *
...
const GRAMMAR = {
  ...
  ".go": "go",
  ".rs": "rust",
  // Godot's two languages. `tree-sitter-wasms` carries neither, so the plugin
  // ships both builds (see `grammarCandidates`).
  ".gd": "gdscript",
  ".tscn": "godot_resource",
  ".tres": "godot_resource",
};
...
function symbolName(node, spec) {
  ...
  if (first !== null && first !== undefined && /identifier$/.test(first.type)) return first.text;
  return null;
}
...
function sectionKind(section) {
  for (const child of section.namedChildren) if (child.type === "identifier") return child.text;
  return null;
}
...
function attributeText(section, key) {
  for (const child of section.namedChildren) {
    ...
    if (name === undefined || value === undefined || name.text !== key) continue;
  ...
  const visit = (node) => {
    if (node.type === "section") {
      ...
      if (kind === "node") {
        ...
        if (name !== null) symbols.push({ name, kind: "sub_resource", line, endLine });
      } else if (kind === "gd_resource") {
        const name = attributeText(node, "type");
        if (name !== null) symbols.push({ name, kind: "resource", line, endLine });
      }
    }
...
/**
 * Where one grammar's wasm may live, best first: the build this plugin ships,
 * then the optional `tree-sitter-wasms` package.
 *
 * The shipped directory exists for Godot, whose languages the package has no
 * grammars for: `lib/grammars/` holds `tree-sitter-gdscript.wasm` and
 * `tree-sitter-godot_resource.wasm`, both compiled from the published grammars
 * by `scripts/build-godot-grammars.mjs`, and both tried before the package.
 */
function grammarCandidates(grammar) {
  const candidates = [join(dirname(fileURLToPath(import.meta.url)), "grammars", `tree-sitter-${grammar}.wasm`)];
  try {
    candidates.push(require.resolve(`tree-sitter-wasms/out/tree-sitter-${grammar}.wasm`));
  } catch {
    // Optional dependency: a grammar shipped with the plugin needs nothing here.
  }
  return candidates;
}
...
export function createTsAnalyzer() {
  if (analyzerPromise !== null) return analyzerPromise;
  analyzerPromise = (async () => {
    ...
    if (typeof Parser !== "function" || typeof Parser.init !== "function") return null;
    ...
    if (typeof Language?.load !== "function") return null;

    const parsers = new Map();
    const loadParser = async (grammar) => {
      if (parsers.has(grammar)) return parsers.get(grammar);
      let parser = null;
    ...
    return {
      ...
      // nothing.
      grammars: [...new Set(Object.values(GRAMMAR))].filter((grammar) =>
        grammarCandidates(grammar).some((wasm) => existsSync(wasm)),
      ),
      ...
      async analyze(text, ext) {
        ...
        try {
          ...
          if (tree.rootNode.hasError === true) return null;
