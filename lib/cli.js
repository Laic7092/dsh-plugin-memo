...
import { buildIndex, buildMap, fileDetail, findInIndex, indexMeta, refreshIndex, staleFiles, TS_MIN_TOKENS } from "./indexer.js";
import { appendNote, readNotes } from "./journal.js";
import { patchStatus, readStatus, sectionBody, STATUS_SECTIONS, writeStatus } from "./status.js";
import { INDEX_MAX_BYTES, clampInt, createMemoDir, findProjectRoot, isFile, memoPaths, readJson, stamp, statOrNull, writeJsonAtomic } from "./store.js";
...
function renderStatusText(project, paths, status, notes, bugs) {
    ...
    if (status.present) {
        ...
        for (const section of extra)
            out.push(`## ${section.title}`, section.body || "（空）", "");
    }
    ...
    for (const note of notes.notes)
        out.push(`  ${note.at}  [${note.kind}]  ${note.text}`);
    return `${out.join("\n").replace(/\n{3,}/g, "\n\n").trimEnd()}\n`;
}
function renderBugs(term, found) {
    if (found.length === 0)
        return `没有匹配 "${term}" 的历史修复。`;
    const out = [`匹配 "${term}"：${found.length} 条`, ""];
    for (const bug of found) {
        out.push(`${bug.id}  ×${bug.occurrences ?? 1}  ${bug.error_message}`);
        if (bug.root_cause)
            out.push(`  cause: ${bug.root_cause}`);
        if (bug.fix)
            out.push(`  fix:   ${bug.fix}`);
        if (bug.file)
            out.push(`  file:  ${bug.file}${Number.isFinite(bug.line) ? ":" + bug.line : ""}`);
        if (Array.isArray(bug.tags) && bug.tags.length > 0)
            out.push(`  tags:  ${bug.tags.join(", ")}`);
        out.push("");
    }
    return `${out.join("\n").trimEnd()}\n`;
}
function renderFileDetail(detail) {
    ...
    if (detail.description)
        out.push(detail.description);
    out.push("");
    if (detail.symbols.length === 0)
        out.push("（这个文件里没有提取到符号）");
...
function renderMap(map) {
    if (map.mode === "rollup") {
        ...
        if (map.truncated)
            out.push("", `（另有 ${map.total - map.dirs.length} 个目录未列出）`);
        return `${out.join("\n").trimEnd()}\n`;
    }
    if (map.files.length === 0)
        return `没有匹配 "${map.focus}" 的文件。`;
    const out = [
...
function renderScan(paths, index, stale, durationMs) {
    ...
    if (stale.changedCount > 0 || stale.missingCount > 0) {
        ...
        for (const rel of stale.changed.slice(0, 5))
            out.push(`  改过  ${rel}`);
        for (const rel of stale.missing.slice(0, 5))
            out.push(`  没了  ${rel}`);
    }
    else {
        out.push("索引与磁盘一致。");
...
function renderSync(sync) {
    if (sync === null || sync === undefined)
        return "";
    if (sync.failed !== undefined)
        return `（索引自动同步失败：${sync.failed} —— 用 memo scan 重建）`;
    if (sync.rebuilt === true)
        return `（索引格式已升级，整份重建：${fmt(sync.changed)} 个文件）`;
    if (sync.changed === 0)
        return "";
    const parts = [];
    if (sync.added.length > 0)
        parts.push(`+${sync.added.length} 新文件`);
    if (sync.updated.length > 0)
        parts.push(`~${sync.updated.length} 改写`);
    if (sync.removed.length > 0)
        parts.push(`-${sync.removed.length} 移除`);
...
function loadIndex(paths) {
    if (!isFile(paths.index))
        return { ok: false, index: null, error: `${paths.index} 还不存在——先跑 memo scan` };
    ...
    if (!file.ok)
        return { ok: false, index: null, error: file.error };
...
async function openIndex(paths, state, analyzer) {
    ...
    if (!loaded.ok || !state.refresh)
        return { ...loaded, sync: null };
    let fresh;
    try {
    ...
    if (fresh.changed > 0) {
        ...
        if (!write.ok)
            return { ok: true, index: fresh.index, error: null, sync: { ...fresh, writeError: write.error } };
    }
...
function coerceFlag(value, flag, given) {
    if (flag.kind === "int") {
        ...
        if (!Number.isFinite(parsed))
            return { ok: false, error: `--${given} 要一个整数，收到 ${JSON.stringify(value)}` };
...
export function splitCommandLine(line) {
    ...
    if (quote !== null)
        return { ok: false, error: `引号没闭合：${quote}` };
    if (current !== null)
        tokens.push(current);
    ...
    for (let i = 0; i < tokens.length; i += 1) {
        ...
        if (token.startsWith("--") && token.length > 2) {
            ...
            if (canonical === undefined)
                return { error: `不认识的选项 --${name}` };
            const flag = known[canonical];
            let value = inline;
            if (value === null) {
                if (i + 1 >= tokens.length)
                    return { error: `--${name} 后面缺一个值` };
                i += 1;
                value = tokens[i];
            }
            ...
            if (flag.kind === "list")
                flags[canonical] = [...(flags[canonical] ?? []), coerced.value];
            else
                flags[canonical] = coerced.value;
            continue;
        }
        // A bare `-` or a negative number is text, not a flag.
        if (token.startsWith("-") && token.length > 1 && !/^-\d/.test(token))
            return { error: `不认识的选项 ${token}` };
        rest.push(token);
    }
    return { flags, rest };
}
...
export const MEMO_COMMANDS = [
    {
        ...
        run(ctx, args) {
            ...
            return renderStatusText(ctx.project, paths, readStatus(paths), readNotes(paths, clampInt(args.flags.notes, 1, 50, 5)), loadBugs(paths));
        },
    },
    {
        name: "handoff",
        ...
        run(ctx, args) {
            ...
            const at = stamp();
            const updated = patchStatus(existing.present ? existing.text : "", basename(project.root) || project.root, { 现在在哪: args.flags.now, 下一步: args.flags.next, 未决问题: args.flags.open, 不要重犯: args.flags.avoid }, at);
            const write = writeStatus(paths, updated);
            if (!write.ok)
                return { ok: false, text: `写 ${paths.status} 失败：${write.error}` };
    ...
    },
    {
        ...
        run(ctx, args) {
            ...
            if (text.length === 0)
                return { ok: false, text: "note 需要一句话：memo note <text> [--kind decision]" };
            ...
            if (!write.ok)
                return { ok: false, text: `写 ${paths.journal} 失败：${write.error}` };
    ...
    },
    {
        ...
        async run(ctx, args) {
            ...
            if (!write.ok)
                return { ok: false, text: `写 ${paths.index} 失败：${write.error}` };
    ...
    },
    {
        ...
        run(ctx, args) {
            ...
            if (term.length === 0)
                return { ok: false, text: "bug-search 需要一个词：memo bug-search <症状或报错文本>" };
            const paths = memoPaths(ctx.project.root, ctx.state.dirName);
            const bugs = loadBugs(paths);
            if (!bugs.ok)
                return `这个项目还没有 bug 记忆：${bugs.error}。修完 bug 后用 memo bug-log 记第一条。`;
    ...
    },
    {
        ...
        run(ctx, args) {
            ...
            if (!result.ok)
                return { ok: false, text: `写 ${paths.bugs} 失败：${result.error}` };
...
export function usageIndex(state = {}) {
    ...
    out.push("", "memo help <子命令>     看一个子命令的全部选项", "--root PATH            指到别的项目（默认：会话工作目录）", "引号                   text 里有空格就加引号，例如 memo note \"换掉了 zod\"");
    return `${out.join("\n").trimEnd()}\n`;
}
//#endregion
...
export async function runMemo(line, env) {
    ...
    if (!split.ok)
        return { ok: false, text: `${split.error}（命令行的引号要成对）` };
    ...
    if (parsed.help === true)
        return commandHelp(command.name);
