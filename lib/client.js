	...
	factory: (require) => {
			...
			return card(
				...
				[
					h(
						...
						field("估算 tokens", num(index.totalTokens)),
		...
		function MemoView(props) {
			...
			react.useEffect(() => {
				...
				setError(null);
				setNotice(null);
				ask("/memo/state", { root: root, refresh: true })
					.then((body) => {
						if (!cancelled) {
							setData(body);
		...
		function readCard(config, saving, save) {
			...
			const state = `（拦截${config.readGuard === true ? "开" : "关"} · 复核${config.refresh === true ? "开" : "关"}）`;
		...
		// 400 on every mount.
		exports.switchConfig = { readStoredConfig: readStoredConfig, writeStoredConfig: writeStoredConfig, patchFor: patchFor, sameConfig: sameConfig, toolMap: toolMap };
