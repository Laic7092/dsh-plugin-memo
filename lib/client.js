 ...
 * guard and refresh policy, the index's exclude list, and one on/off per tool —
 * plus "rebuild index", which the host refuses for a directory that has no
 * `.memo/` yet.
 */
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 * panelConfig.
		 *
		 * What is remembered is a *patch* — `tools` as a name -> boolean map —
		 * while the host reports `tools` as the grouped catalogue the card
		 * renders. The two shapes are reconciled by `toolMap`/`patchFor` below
		...
		function readStoredConfig() {
			...
			try {
				...
				const kept = {};
				if (typeof parsed.readGuard === "boolean") kept.readGuard = parsed.readGuard;
				if (typeof parsed.refresh === "boolean") kept.refresh = parsed.refresh;
				if (parsed.tokenizer === "exact" || parsed.tokenizer === "estimated") kept.tokenizer = parsed.tokenizer;
				if (Array.isArray(parsed.exclude)) kept.exclude = parsed.exclude.filter((name) => typeof name === "string");
				if (parsed.tools !== null && typeof parsed.tools === "object" && !Array.isArray(parsed.tools)) {
					const tools = {};
					for (const [name, on] of Object.entries(parsed.tools)) if (typeof on === "boolean") tools[name] = on;
					if (Object.keys(tools).length > 0) kept.tools = tools;
				}
				return kept;
			} catch {
				return {};
		...
		function writeStoredConfig(config) {
			const kept = {};
			if (typeof config.readGuard === "boolean") kept.readGuard = config.readGuard;
			if (typeof config.refresh === "boolean") kept.refresh = config.refresh;
			if (config.tokenizer === "exact" || config.tokenizer === "estimated") kept.tokenizer = config.tokenizer;
			if (Array.isArray(config.exclude)) kept.exclude = config.exclude.slice();
			const tools = toolMap(config);
			if (Object.keys(tools).length > 0) kept.tools = tools;
			writeStored(CONFIG_KEY, JSON.stringify(kept));
		}

		/** The host's grouped tool switches, flattened to `{ name: on }`. */
		function toolMap(config) {
			const map = {};
			if (config === null || typeof config !== "object" || !Array.isArray(config.tools)) return map;
			for (const group of config.tools) {
				if (group === null || typeof group !== "object" || !Array.isArray(group.tools)) continue;
				for (const entry of group.tools) if (entry && typeof entry.name === "string") map[entry.name] = entry.on === true;
			}
			return map;
		}
		...
		function patchFor(wanted, live) {
			...
			const patch = {};
			if (typeof live.readGuard === "boolean" && typeof wanted.readGuard === "boolean") patch.readGuard = wanted.readGuard;
			if (typeof live.refresh === "boolean" && typeof wanted.refresh === "boolean") patch.refresh = wanted.refresh;
			if (typeof live.tokenizer === "string" && typeof wanted.tokenizer === "string") patch.tokenizer = wanted.tokenizer;
			if (Array.isArray(live.exclude) && Array.isArray(wanted.exclude)) patch.exclude = wanted.exclude;
			if (Array.isArray(live.tools) && wanted.tools !== null && typeof wanted.tools === "object") {
				const known = toolMap(live);
				const tools = {};
				for (const [name, on] of Object.entries(wanted.tools)) if (name in known && typeof on === "boolean") tools[name] = on;
		...
		function sameConfig(patch, live) {
			if ("readGuard" in patch && patch.readGuard !== live.readGuard) return false;
			if ("refresh" in patch && patch.refresh !== live.refresh) return false;
			if ("tokenizer" in patch && patch.tokenizer !== live.tokenizer) return false;
			if ("exclude" in patch && (patch.exclude || []).join("\u0000") !== (live.exclude || []).join("\u0000")) return false;
			if ("tools" in patch) {
				const liveTools = toolMap(live);
				for (const [name, on] of Object.entries(patch.tools)) if (liveTools[name] !== on) return false;
			}
			return true;
		}
		...
		function ask(path, options) {
			...
			if (settings.root) query.set("root", settings.root);
			if (settings.refresh === true) query.set("refresh", "1");
			if (settings.tokenizer === "exact" || settings.tokenizer === "estimated") query.set("tokenizer", settings.tokenizer);
			...
			return window
				...
				.then((response) =>
					response.json().then(
						(body) => {
							if (!body || body.ok !== true) throw new Error((body && body.error) || `HTTP ${response.status}`);
							return body;
						},
						() => {
		...
		function toggle(label, hint, checked, disabled, onChange) {
			return h(
				"label",
				{ className: "memoToggle", key: label },
				h("input", { type: "checkbox", checked: checked, disabled: disabled, onChange: (event) => onChange(event.target.checked) }),
				h(
					"span",
					{ className: "memoToggleText" },
					h("span", { className: "memoToggleLabel" }, label),
					hint ? h("span", { className: "memoToggleHint" }, hint) : null,
				),
			);
		}
		//#endregion
		...
		function indexCard(index) {
			if (index.present !== true) return card("代码索引", prose("还没有索引。在项目里跑一次 memo_scan 就会建起来。", "memoMuted"));
			if (index.readable !== true) return card("代码索引", prose(index.error || "index.json 读不出来。", "memoErr"));
			...
			return card(
				...
				[
					...
					stale
						? h(
								"p",
								{ className: "memoBody memoWarn", key: "stale" },
								`索引落后于磁盘：${index.staleChanged} 个文件改过、${index.staleMissing} 个已消失。` +
		...
		function statusCard(status) {
			if (status.present !== true) return card("STATUS.md", prose("还没有 STATUS.md。让模型调一次 memo_handoff 就有第一份。", "memoMuted"));
			...
			for (const section of status.sections.concat(status.extra)) {
				blocks.push(
					h(
						"section",
						{ key: `s-${section.title}`, className: "memoSection" },
						h("h4", null, section.title),
						h("p", { className: section.body ? "memoBody" : "memoBody memoMuted" }, section.body || "（空）"),
					),
				);
			}
		...
		function journalCard(journal) {
			if (journal.present !== true || journal.notes.length === 0) return card("最近动作", prose("journal 还是空的。", "memoMuted"));
			return card(
				`最近动作（${journal.notes.length} / ${num(journal.total)}）`,
				h(
					"ul",
					{ className: "memoList" },
					journal.notes.map((note, index) =>
						h(
							"li",
							{ key: `${note.at}-${index}` },
							h("span", { className: "memoWhen" }, String(note.at || "").replace("T", " ").slice(0, 16)),
							h("span", { className: "memoTag" }, note.kind || "note"),
							h("span", { className: "memoText" }, note.text),
						),
					),
				),
			);
					...
					: h(
							...
							bugs.recent.map((bug) =>
								h(
									"li",
									{ key: bug.id },
									h("span", { className: "memoTag" }, `${bug.id} ×${bug.occurrences}`),
									h("span", { className: "memoText" }, bug.errorMessage, bug.fix ? h("span", { className: "memoMuted" }, ` — ${clamp(bug.fix, 160)}`) : null),
								),
							),
						),
			);
		...
		function MemoView(props) {
			...
			// the host revalidate the index the way memo_find does, so the staleness
			...
			// it also says which tools exist here — the patch is filtered against
			...
			react.useEffect(() => {
				...
				ask("/memo/config")
					.then((body) => {
						...
						if (Object.keys(patch).length === 0 || sameConfig(patch, body.config)) return null;
						return ask("/memo/config", { method: "POST", body: patch }).then((saved) => {
							if (!cancelled) setConfig(saved.config);
						});
					})
					.catch(() => {
			...
			const bar = h(
				"form",
				{ className: "memoBar", onSubmit: open },
				h("input", {
					className: "memoInput",
					type: "text",
					value: draft,
				...
				h("button", { className: "memoBtn", type: "submit", disabled: busy !== null }, "打开"),
				h("button", { className: "memoBtn", type: "button", disabled: busy !== null, onClick: () => setReloadKey((key) => key + 1) }, busy === "load" ? "读取中…" : "刷新"),
				h(
					"button",
					{ className: "memoBtn memoBtnPrimary", type: "button", disabled: busy !== null || data === null || data.initialized !== true, onClick: scan },
					busy === "scan" ? "重建中…" : "重建索引",
				),
			...
			const body =
				data.initialized !== true
					? [prose("这个目录里还没有 .memo/。在项目里让模型跑一次 memo_scan（或 /memo），面板才能接管。", "memoMuted")]
		...
		/**
		 * What each tool is called on screen, and what a person is trading away by
		 * leaving it on. The host names the tools and reports their state; the
		 * words live here, beside the rest of this file's copy, and a name this
		 * map has never heard of falls back to showing the name itself.
		 */
		const TOOL_TEXT = {
			memo_status: ["读状态", "先看 STATUS、日志和 bug 数，再动代码"],
			memo_handoff: ["写交接", "更新 STATUS.md 里你指定的那几节"],
			memo_note: ["记一行", "往 journal.jsonl 追加一条"],
			memo_bug_search: ["查缺陷", "按症状找以前记下的根因和修法"],
			memo_bug_log: ["记缺陷", "把根因和修法写进 bugs.json"],
			memo_scan: ["建索引", "全量扫描并写 index.json"],
			memo_find: ["找符号", "按符号或路径定位到行号"],
			memo_map: ["看地图", "目录分布，或聚焦某个主题的文件清单"],
		};
		const GROUP_TEXT = { memory: "项目记忆", index: "代码索引", bugs: "缺陷记忆" };

		/**
		 * The tool switches, grouped the way the host groups them.
		 *
		 * Turning one off takes the tool out of this process's tool table, so it
		 * is not a display filter and the card says as much: the model can no
		 * longer see or call it. The group button is the same write over the
		 * group's whole set, which is why it can send several keys at once.
		 */
		function toolsCard(config, saving, save) {
			const groups = Array.isArray(config.tools) ? config.tools : [];
			if (groups.length === 0) {
				return card("工具开关", prose("这个宿主半没有报告工具清单（版本较旧，或者路由没注册）。", "memoMuted"));
			}
			const blocks = [];
			let on = 0;
			let total = 0;
			for (const group of groups) {
				const entries = Array.isArray(group.tools) ? group.tools : [];
				...
				blocks.push(
					h(
						"div",
						{ key: `group-${group.id}`, className: "memoGroup" },
						h("span", { className: "memoGroupName" }, title, h("span", { className: "memoGroupCount" }, `　${onCount}/${entries.length}`)),
						entries.length === 0
							? null
							: h(
									"button",
									{
										...
										onClick: () => {
											const patch = {};
											for (const entry of entries) patch[entry.name] = !allOn;
											save({ tools: patch });
										},
									},
									allOn ? "全关" : "全开",
				...
				for (const entry of entries) {
					const text = TOOL_TEXT[entry.name] || [];
					blocks.push(toggle(text[0] || entry.name, text[1], entry.on === true, saving, (value) => save({ tools: { [entry.name]: value } })));
				}
			}
			blocks.push(
				h(
					"p",
					{ key: "note", className: "memoBody memoMuted" },
					"关掉的工具会从这个进程的工具表里移除，模型看不到也调不到它；重启 profile 后回到 composition 里的值，这个页面会在下次打开时重新应用你选过的。",
				),
			);
			return foldCard(`工具开关（${on}/${total} 开启）`, blocks);
		}
		...
		function readCard(config, saving, save) {
			...
			// The summary states the switches, the way the tool card's summary
			...
			return foldCard(`读取与刷新${state}`, [
				...
				toggle(
					"用 DeepSeek V4 分词器精确计数",
					"开：每个 token 数都是真分词器的输出（首次计数要读 6MB 词表）。关：仍是 ~4 字符一个 token 的估算。两种都会写进索引，切换后下次扫描/复核会重建",
					config.tokenizer === "exact",
					saving,
					(value) => save({ tokenizer: value ? "exact" : "estimated" }),
				),
				h(
					"div",
					{ key: "exclude", className: "memoBar" },
					h("input", {
						className: "memoInput",
						type: "text",
		...
		function configCardFor(rawConfig, saving, save) {
			...
			return [toolsCard(config, saving, save), readCard(config, saving, save)];
		}
		...
		exports.switchCards = { toolsCard: toolsCard, readCard: readCard };
		...
		// 400 on every mount.
		exports.switchConfig = { readStoredConfig: readStoredConfig, writeStoredConfig: writeStoredConfig, patchFor: patchFor, sameConfig: sameConfig, toolMap: toolMap, ask: ask };
