...
function resolveTokenCounter(options, log) {
    ...
    if (typeof asked === "function")
        return { mode: "custom", count: asked };
    if (asked === "exact")
        return { mode: "exact", count: exactTokens };
...
function braceEnd(lines, start) {
    ...
    if (!stripLine(lines[start]).includes("{"))
        return start;
    let depth = 0;
    let seen = false;
...
function sectionEnd(lines, start) {
    const trim = (end) => {
        let at = end;
        while (at > start && lines[at].trim().length === 0)
            at -= 1;
        return at;
    };
    for (let i = start + 1; i < lines.length; i++)
...
function endFor(language, lines, start) {
    if (language === "py")
        return indentEnd(lines, start);
    if (language === "gd")
        return gdBodyEnd(lines, start);
    if (language === "gdres")
        return sectionEnd(lines, start);
...
function extractClassMembers(lines) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        while (stack.length > 0 && depth < stack[stack.length - 1].memberDepth)
            stack.pop();
        ...
        if (head !== null)
            stack.push({ name: head[1], memberDepth: depth });
    }
    return members;
}
/**
...
function extractGdResourceSymbols(lines) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (resource !== null)
            symbols.push({ name: resource[1], kind: "resource", line: i + 1, endLine: sectionEnd(lines, i) + 1 });
    }
    return symbols;
}
function extractSymbols(lines, language) {
    if (language === "gdres")
        return extractGdResourceSymbols(lines);
    ...
    if (language === "js")
        symbols.push(...extractClassMembers(lines));
...
function walk(root, excludes, files, depth = 0) {
    if (files.length >= MAX_FILES || depth > 24)
        return;
    let entries;
    try {
    ...
    for (const entry of entries) {
        ...
        if (entry.name.startsWith(".") && entry.name !== ".") {
            ...
            if (entry.isDirectory())
                continue; // dot-directories are configuration, not source
        }
...
async function indexOneFile(root, full, language, ext, maxBytes, analyzer, counter) {
    ...
    if (text.includes("\u0000"))
        return null; // binary that happens to carry a source extension
...
export async function refreshIndex(index, options = {}) {
    ...
    for (const { full, language, ext } of found) {
        ...
        if (one === null)
            continue; // over the cap, binary, or unreadable
        files[one.rel] = one.entry;
        if (before === undefined)
            added.push(one.rel);
        else
            updated.push(one.rel);
    }
...
function rankByImportance(files) {
    ...
    for (const name of names) {
        ...
        if (typeof declared === "string" && declared.length > 0 && !byClassName.has(declared))
            byClassName.set(declared, name);
    }
    ...
    for (let iteration = 0; iteration < 20; iteration++) {
        ...
        for (const name of names)
            next.set(name, next.get(name) + spread);
        rank = next;
    }
...
function hitCost(relPath, name, tokens) {
...
export function findInIndex(index, query, options = {}) {
    ...
    if (needle.length === 0)
        return { matches: [], spent: 0, budget, truncated: false, total: 0 };
    const candidates = [];
    for (const [relPath, file] of Object.entries(index.files)) {
        ...
        if (best === 0 && (file.description ?? "").toLowerCase().includes(needle))
            best = 12;
        if (best === 0)
            continue;
        candidates.push({
    ...
    for (const candidate of candidates) {
        ...
        if (spent + cost > budget && matches.length > 0)
            break;
        spent += cost;
...
export function buildMap(index, focus, options = {}) {
    ...
    for (const entry of scored) {
        ...
        if (spent + cost > budget && files.length > 0)
            break;
        spent += cost;
        files.push(entry);
    }
