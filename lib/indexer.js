 ...
 */
async function indexOneFile(root, full, language, ext, maxBytes, analyzer) {
  let stat;
  try {
    stat = statSync(full);
  } catch {
  ...
  if (analyzer !== null && tokens >= TS_MIN_TOKENS) {
    try {
...
export async function buildIndex(root, options = {}) {
  ...
  for (const { full, language, ext } of found) {
    const one = await indexOneFile(resolved, full, language, ext, maxBytes, analyzer);
 ...
 * @param options - `exclude`, `maxFileBytes`, `analyzer`, and an optional `root` override.
...
export async function refreshIndex(index, options = {}) {
  ...
  if (
    ...
    const rebuilt = await buildIndex(root, { exclude: requested, maxFileBytes: maxBytes, analyzer });
    return {
      index: rebuilt,
  ...
  for (const { full, language, ext } of found) {
    ...
    const one = await indexOneFile(root, full, language, ext, maxBytes, analyzer);
...
export function findInIndex(index, query, options = {}) {
  ...
  for (const candidate of candidates) {
    const cost = hitCost(candidate.relPath, candidate.symbol ? candidate.symbol.name : null);
...
export function buildMap(index, focus, options = {}) {
  ...
  if (terms.length === 0) {
    ...
    for (const bucket of ranked) {
      const cost = estimateTokens(`${bucket.dir} ${bucket.best?.relPath ?? ""} ${bucket.best?.description ?? ""}`) + 6;
      if (spent + cost > budget && out.length > 0) break;
      spent += cost;
      out.push(bucket);
    }
    return { mode: "rollup", focus: null, dirs: out, spent, budget, truncated: out.length < ranked.length, total: ranked.length };
  }
  ...
  for (const entry of scored) {
    const cost = estimateTokens(`${entry.relPath} ${entry.description ?? ""}`) + 8;
    if (spent + cost > budget && files.length > 0) break;
    spent += cost;
    files.push(entry);
  }
  return { mode: "focused", focus: terms.join(" "), files, spent, budget, truncated: files.length < scored.length, total: scored.length };
}
