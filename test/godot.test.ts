 ...
 * `tree-sitter-wasm` ships both of Godot's languages, so the plugin needs no
 * wasm of its own; those tests skip when the optional package, or one of its
 * parsers, is missing rather than pretending the ceiling is what they measured.
 */
...
async function godotAnalyzer(t, grammar) {
  ...
  if (!analyzer.grammars.includes(grammar)) {
    t.skip(`tree-sitter-wasm's ${grammar} parser is missing`);
    return null;
  }
  return analyzer;
}
